package Testing;

import java.util.List;

import org.openqa.selenium.By;
import org.openqa.selenium.WebElement;

public class PrintingAlltableheader extends Facebookj {
	
	public void display() {
		List<WebElement> tablenames=driver.findElements(By.xpath("//table[@class='dataTable']/thead/tr/th"));
		for(int i=0;i<tablenames.size();i++) {
			System.out.println(tablenames.get(i).getText());
		}
	}

	public static void main(String[] args) {
		 PrintingAlltableheader a = new PrintingAlltableheader();
		 a.LaunchBrowser();
		 a.NavigateURL("https://demo.guru99.com/test/web-table-element.php");
		 System.out.println("title of the page"+a.getTitle());
		 System.out.println("title of the url"+a.getCurrentURL());
		 a.display();
		 
	}
		
		

}
